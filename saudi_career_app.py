...
st.markdown("""
<style>
.block-container{max-width:1250px;padding-top:3.4rem;padding-bottom:4rem}
...
.lang-label{font-size:.82rem;opacity:.72;margin-bottom:.15rem;text-align:center}
div[data-testid="stRadio"] > div{justify-content:center;gap:.8rem}
@media (max-width: 768px){.block-container{padding-top:4rem}.hero{padding:1.5rem}.hero h1{font-size:2rem}}
</style>
""", unsafe_allow_html=True)

lang_space, lang_box = st.columns([5.3, 1.7])
with lang_box:
    st.markdown('<div class="lang-label">اللغة / Language</div>', unsafe_allow_html=True)
    lang = st.radio("Language / اللغة", ["العربية", "English"], horizontal=True, label_visibility="collapsed", key="language_switch")
AR = lang == "العربية"
...
major_ar = {
"Statistics":"الإحصاء","Data Science":"علوم البيانات","Computer Science":"علوم الحاسب","Information Systems":"نظم المعلومات","Information Technology":"تقنية المعلومات","Artificial Intelligence":"الذكاء الاصطناعي","Cybersecurity":"الأمن السيبراني","Software Engineering":"هندسة البرمجيات","Mathematics":"الرياضيات","Actuarial Science":"العلوم الاكتوارية","Economics":"الاقتصاد","Finance":"المالية","Accounting":"المحاسبة","Business Administration":"إدارة الأعمال","Marketing":"التسويق","Digital Marketing":"التسويق الرقمي","Human Resources":"الموارد البشرية","Supply Chain Management":"إدارة سلاسل الإمداد","Operations Management":"إدارة العمليات","Operations and Information Management":"إدارة العمليات والمعلومات","Industrial Engineering":"الهندسة الصناعية","Electrical Engineering":"الهندسة الكهربائية","Mechanical Engineering":"الهندسة الميكانيكية","Civil Engineering":"الهندسة المدنية","Chemical Engineering":"الهندسة الكيميائية","Architecture":"العمارة","Law":"القانون","Public Administration":"الإدارة العامة","Healthcare Administration":"الإدارة الصحية","Medicine":"الطب","Nursing":"التمريض","Pharmacy":"الصيدلة","Public Health":"الصحة العامة","Education":"التربية","English Language":"اللغة الإنجليزية","Arabic Language":"اللغة العربية","Media and Communication":"الإعلام والاتصال","Graphic Design":"التصميم الجرافيكي","Quantitative Methods":"الأساليب الكمية","Other":"أخرى"
}

# Product heuristic used only for major-to-career-family alignment. It is not an official taxonomy.
major_family_map = {
"Statistics":["Data & Analytics","Finance & Accounting"],"Data Science":["Data & Analytics","Technology"],"Computer Science":["Technology","Data & Analytics"],"Information Systems":["Technology","Data & Analytics","Administration & Support"],"Information Technology":["Technology"],"Artificial Intelligence":["Technology","Data & Analytics"],"Cybersecurity":["Technology"],"Software Engineering":["Technology"],"Mathematics":["Data & Analytics","Education"],"Actuarial Science":["Finance & Accounting","Data & Analytics"],"Economics":["Finance & Accounting","Data & Analytics"],"Finance":["Finance & Accounting"],"Accounting":["Finance & Accounting"],"Business Administration":["Administration & Support","Sales & Marketing","Human Resources","Operations & Supply Chain"],"Marketing":["Sales & Marketing"],"Digital Marketing":["Sales & Marketing"],"Human Resources":["Human Resources","Administration & Support"],"Supply Chain Management":["Operations & Supply Chain"],"Operations Management":["Operations & Supply Chain","Administration & Support"],"Operations and Information Management":["Operations & Supply Chain","Technology","Data & Analytics"],"Industrial Engineering":["Engineering","Operations & Supply Chain","Data & Analytics"],"Electrical Engineering":["Engineering","Technology"],"Mechanical Engineering":["Engineering"],"Civil Engineering":["Engineering"],"Chemical Engineering":["Engineering"],"Architecture":["Engineering"],"Law":["Legal & Compliance"],"Public Administration":["Administration & Support"],"Healthcare Administration":["Healthcare","Administration & Support"],"Medicine":["Healthcare"],"Nursing":["Healthcare"],"Pharmacy":["Healthcare"],"Public Health":["Healthcare","Data & Analytics"],"Education":["Education"],"English Language":["Education","Administration & Support"],"Arabic Language":["Education","Administration & Support"],"Media and Communication":["Sales & Marketing"],"Graphic Design":["Sales & Marketing"],"Quantitative Methods":["Data & Analytics"],"Other":[]
}
...
with tab1:
    ...
    aligned_families = major_family_map.get(major, [])
    major_score = 100.0 if target in aligned_families else (60.0 if major == "Other" else 35.0)
    # Major is deliberately part of the score so changing it changes the result.
    score=round(.35*market+.25*exp_score+.15*city_score+.10*sal_score+.15*major_score,1)

    with right:
        st.markdown("### "+t("نتيجتك","Your result"))
        a,b=st.columns(2)
        a.metric(t("مؤشر ملاءمة السوق","Market-fit score"),f"{score}%")
        b.metric(t("فرص السوق","Market Opportunity"),f"{market:.0f}/100")
        st.progress(min(max(score/100,0),1))
        st.caption(t("يتكوّن المؤشر من: فرص السوق 35%، توافق الخبرة 25%، سياق المدينة 15%، توافق الراتب 10%، ومواءمة التخصص 15%. مواءمة التخصص قاعدة تحليلية في المنتج وليست تصنيفًا رسميًا.",
                     "Score weights: market opportunity 35%, experience alignment 25%, city context 15%, salary alignment 10%, and major alignment 15%. Major alignment is a product heuristic, not an official taxonomy."))
        c,d=st.columns(2)
        ...
        official_major = major in set(major_seed["major"].dropna())
        if official_major and target == "Data & Analytics":
            st.success(t(
            ...
            st.caption(t(
                "مواءمة هذا التخصص مع المجال هنا قاعدة تحليلية في المنتج وليست ادعاءً بوجود ربط رسمي مباشر.",
                "This major-to-family alignment is a product heuristic, not a claim of direct official linkage."
            ))
...
with tab2:
    ...
    with c1:
        ct=(clean[clean["entry_level_flag"]].groupby("city_standard")
            .agg(Postings=("job_title","size"),Median_Salary=("salary_sar","median"),Salary_N=("salary_sar","count"))
            .sort_values("Postings",ascending=False).head(10).reset_index())
        ct["city_standard"]=ct["city_standard"].map(city_label)
        ct["Median_Salary"]=[money(v) if n >= 5 else t("عينة غير كافية","Insufficient sample") for v,n in zip(ct["Median_Salary"],ct["Salary_N"])]
        ...
        st.dataframe(ct[[t("المدينة","City"),t("الإعلانات","Postings"),t("وسيط الراتب","Median salary")]],use_container_width=True,hide_index=True)
    with c2:
        ...
        ft["family_median_salary"] = ft["family_median_salary"].map(money)
