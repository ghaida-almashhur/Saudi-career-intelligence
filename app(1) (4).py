...
with tab1:
    ...
    with left:
        st.text_input(t("التخصص","Major"), value=t("الإحصاء","Statistics"))
...
with tab3:
    ...
    selected_skills=st.multiselect(t("المهارات التي لديك","Skills you already have"),opts)
    ...
    if missing:
        ...
        if len(ranked):
            st.success(t("المهارة التالية المقترحة: ","Prototype next skill: ")+f"**{ranked.iloc[0]['skill_name_en']}**")
...
with tab4:
    ...
    st.dataframe(stats_path[["target_role","fit_level","official_evidence","development_direction"]],
                 use_container_width=True,hide_index=True)
    st.markdown("### "+t("مهارات عائلة التحليل الإحصائي الرسمية","Official Statistical Analysis family skills"))
    st.write(", ".join(stat_skills["skill_name_en"].dropna().tolist()))
